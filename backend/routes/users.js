...
module.exports = (io) => {
  // Your existing router logic here
  ...
  router.get("/messages/:userId/:friendId", verifyToken, async (req, res) => {
  ...
  router.post("/upload", upload.single("image"), (req, res) => {
    if (!req.file) {
      return res
        .status(400)
        .json({ statusCode: 400, error: "No file uploaded", data: "" });
    }
  ...
  return router; // Make sure this is returned
};
